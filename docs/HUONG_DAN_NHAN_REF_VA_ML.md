...
1. **Trích xuất trước** bản đồ góc chuẩn từ video YouTube → lưu trong `database/reference_<bài_tập>.json`  
   Mỗi dòng gồm: `time`, `vai`, `khuyu` — là các **tư thế mẫu** đã quan sát được khi chuyên gia/NCV thực hiện đúng động tác trên video hướng dẫn.
...
3. **Khi phân tích từng frame bệnh nhân**, máy tìm **tư thế mẫu gần nhất** trong JSON (theo góc, không theo giây):
...
4. **Codman:** chỉ so góc **tay phải** (vai phải + khuỷu phải).  
...
|-------------------|------------|-------------------------|
| Codman — [youtu.be/a4eCRWuqO40](https://youtu.be/a4eCRWuqO40) | `reference_codman.json` | Xoay vòng tròn / con lắc tay phải |
| Gậy — [youtube.com/watch?v=s2O8WHT5o2k](https://www.youtube.com/watch?v=s2O8WHT5o2k) | `reference_gay.json` | Nâng gậy, xoay vai ngoài/trong |
